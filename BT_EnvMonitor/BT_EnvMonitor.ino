...
uint32_t RTC_LastReading;
uint32_t RTC_CurrentReading;

const unsigned int BME_READ_INTERVAL = 5;
...
void setup()
{
    ...
    RTC_LastReading = RTC_CurrentReading - BME_READ_INTERVAL;
}
...
void loop()
{
    ...
    displayDate();
    // update the temp data on timer
    RTC_CurrentReading = dt.unixtime();
    if (RTC_CurrentReading - RTC_LastReading >= BME_READ_INTERVAL)
    {
        BME680_DisplayData();
        RTC_LastReading = RTC_CurrentReading;
    }
    ...
    if (BTSerial.available())
    {
        ...
        else if (btData == BT_GET_ENVDATA)
        {
            ...
            /*
            BTSerial.print("ENV:");
            BTSerial.print(bme.temperature); BTSerial.print(";");
            BTSerial.print(bme.humidity);    BTSerial.print(";");
            BTSerial.print(bme.pressure);    BTSerial.print(";");
            BTSerial.println("#");
            */
